...
class ComentProductAPIView(APIView):
    def get(self, request, product_slug):
        ...
        comentarios = Coment_product.objects.filter(product__slugProduct=product_slug).order_by('-created')

        # Serializar los comentarios con el contexto de la solicitud
        serializer = ComentProductSerializers(comentarios, many=True, context={'request': request})
    ...
    def post(self, request: Request, format=None):
        try:
            ...
            if serializer.is_valid():
                ...
                comentarios = Coment_product.objects.filter(product__id=product_slug).order_by('-created')
                comment_serializer = ComentProductSerializers(comentarios, many=True, context={'request': request})

                return Response(
                    {"comment": serializer.data, "comments": comment_serializer.data}, status=status.HTTP_201_CREATED,
                )
...
class ComentDeleteAPIView(generics.DestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if comentario.user != request.user:
            return Response({"message": "No tienes permiso para eliminar este comentario."},
        ...
        return Response({"comments": serializer.data, "message": "El comentario ha sido eliminado exitosamente."},
...
class ComentUpdateAPIView(generics.UpdateAPIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if comentario.user != request.user:
            return Response({"message": "No tienes permiso para editar este comentario."},
                            status=status.HTTP_403_FORBIDDEN)
        ...
        if serializer.is_valid():
            ...
            return Response({"comments": comentarios_serializer.data, "message": "El comentario ha sido actualizado exitosamente."},
                            status=status.HTTP_200_OK)
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)