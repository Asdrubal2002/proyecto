...
class WishListView(APIView):
    ...
    def get(self, request, format=None):
        ...
        wish_list_products = WishList.objects.filter(
            user=user,
            product__is_active=True,  # Filtrar solo productos activos
            product__category__is_active=True,
            product__category__parent__is_active=True,
        ...
        # Devolver la lista de productos de la lista de deseos y el recuento
        return Response(
            {
...
class AddToWishListView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            Product.objects.filter(slugProduct=product_slug).update(
                likes=F("likes") - 1
            )
            ...
            return Response(
                {"wishlist": serializer.data}, status=status.HTTP_201_CREATED
            )
...
class WishListStoresView(APIView):
    ...
    def get(self, request, format=None):
        ...
        # Obtener todos los productos en la lista de deseos del usuario actual
        wish_list_stores = WishListStore.objects.filter(
            user=user, store__is_active=True
        )
...
class AddToWishListStoreView(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            return Response(
                {"wishlist_stores": serializer.data}, status=status.HTTP_200_OK
            )
            ...
            return Response(
                {"wishlist_stores": serializer.data}, status=status.HTTP_201_CREATED
            )
